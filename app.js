...
const inventorNames = inventors.map((inventor,) => {
    let inventorFullName = {};
    inventorFullName["first"] = inventor.first;
    inventorFullName["last"] = inventor.last;
    return inventorFullName;
});

// Simplified version using implicit return with arrow functions:
// Returning an object literal in map: Parentheses are necessary to avoid confusion between an object literal and a function body.
...
// Complete the exercise in the space below:
// Returning a simple value in sort: Parentheses are optional because there’s no ambiguity.
...
const firstLast = people.map((person) => {
    ...
    let name = `${nameArray[1]} ${nameArray[0]}`;
    return name;
})
...
//  // Perform some logic and return the updated accumulator
// }, initialValue);

const totalYearsLived = inventors.reduce((totalYearsLived, inventor) => {
  let lifespan = inventor.passed - inventor.year;
  return totalYearsLived + lifespan;
}, 0);
...
// Complete the exercise in the space below:
const travelMethodCounts = travelMethods.reduce((travelMethodCounts, travelMethod) => {
  if (travelMethod in travelMethodCounts){
    travelMethodCounts[travelMethod] += 1;
  } else {
    travelMethodCounts[travelMethod] = 1;
  }
  return travelMethodCounts;
}, {});
